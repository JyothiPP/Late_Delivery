...
data=pd.read_csv(r"/workspaces/Late_Delivery/DataCo.csv",encoding_errors="ignore")
print("loaded data")
...
data['order date (DateOrders)'] = data['order date (DateOrders)'].apply(lambda x: parser.parse(x).strftime('%Y-%m-%d %H:%M'))
...
to_encode=['Type','Category Name','Customer City','Customer Country','Customer Segment','Customer State','Department Name',
          'Market','Order City','Order Country','Order Region','Order State','Order Status','Product Price','Shipping Mode','is weekend']
